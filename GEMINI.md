...
---
## 🛠 Getting Started (Elixir side)

```bash
mix setup          # installs deps & assets
mix phx.server     # run dev server (or: iex -S mix phx.server)
```
...
- CSV storage layer + tests  
- Sidebar & Chat LiveView (no AI yet)  
- AI streaming integration (dummy client)  
- Visual polish & responsiveness  
- Telemetry hooks & CI tests  
_(Full checklist → PLAN 2 §13)_

---
## 🔧 Generator & Workflow Shortcuts

Refer to `KNOWHOW_tech_stack.md` for detailed examples, but in short:

| Tool | Command | Why |
|------|---------|-----|
| Phoenix LiveView | `mix phx.gen.live … --no-schema` | Scaffold LiveView/template/tests without Ecto |
| Phoenix Component | `mix phx.gen.component …` | Quick HEEx component shells |
| Ash | `mix ash.gen.resource … --data-layer simple` | Generate resource files, then point to `CsvStorage` |

Run generators in a scratch branch, cherry-pick useful parts, and remove Ecto bits to obey the no-DB rule.

---
## 🚦 Guard-Rails & Best Practices

1. **No new dependencies** – modifying `mix.exs` or `assets/package.json` requires explicit approval.  
2. **CSV only** – never add Ecto migrations; persistence logic lives in `CsvStorage`.  
3. **LiveView streams** – use `stream_insert/4` for incremental UI, `:temporary_assigns` to reduce memory.  
4. **Non-blocking** – all external AI calls run in supervised `Task` processes; never block the LiveView process.  
5. **Testing** – favour `Phoenix.LiveViewTest` for UI, unit-test `CsvStorage` and `AIClient` with stubs.

---
## 📚 Further Reading & Local Docs

- Project plans: [`PLAN_0_description.md`](PLAN_0_description.md), [`PLAN_1_High_Level.md`](PLAN_1_High_Level.md), [`PLAN_2_implementation_howto.md`](PLAN_2_implementation_howto.md)
- Generator tips: [`KNOWHOW_tech_stack.md`](KNOWHOW_tech_stack.md)
- Offline reference docs shipped in repo:
  - `docs/Elixir/` – BEAM, OTP, concurrency patterns
  - `docs/Phoenix/` – controllers, channels, sockets, Verified Routes
  - `docs/PhoenixLiveView/` – `live_stream/4`, hooks, testing helpers
  - `docs/ash/` – manual actions, policies, Ash AI, auth
- Upcoming guide: `AIChatUIDataStream.md` will consolidate implementation guidance.

## ⛏️ Command Summary (monorepo)

| Area | Command |
|------|---------|
| Elixir tests | `mix test` |
| Formatter check | `mix format --check-formatted` |
| Dev server | `mix phx.server` |
| Node preflight (if touching JS assets) | `npm run preflight` |
